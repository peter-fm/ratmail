...
impl FileExplorer {
    ...
    pub fn handle<I: Into<Input>>(&mut self, input: I) -> Result<()> {
        ...
        match input {
            ...
            Input::Right => {
                if let Some(current) = self.files.get(self.selected) {
                    if current.path.is_dir() && !current.is_placeholder() {
    ...
    /// Sets the filter string and refreshes the file list.
    #[inline]
    ...
    /// Clears the filter string and refreshes the file list.
    #[inline]
    ...
    fn apply_filter(&mut self) {
        ...
        if filter.is_empty() {
            files.extend(self.all_files.iter().cloned());
        } else {
            files.extend(
                self.all_files
                    .iter()
                    .filter(|file| file.name.to_ascii_lowercase().contains(&filter))
                    .cloned(),
            );
        }
...
impl File {
    ...
    /// Returns `true` if the file is a placeholder (e.g. no matches).
    #[inline]
    #[must_use]
